...
var xtoys_speed_zero: bool = false
...
func stop_xtoys():
	...
	if ws_server:
		ws_server.stop()
		ws_server = null
		_log("XToys WebSocket server stopped")
	else:
		_log("No XToys WebSocket server to stop")
	xtoys_running = false
	_stop_speed_mode()
	_log("xtoys bridge stopped")

func _process(delta):
	if %BridgeControls.xtoysenabled == true:
		...
		while message_queue.size() > 0 and processed < 10:
			...
			_log("[DEBUG] Processing queued xtoys message: %s" % str(result))
			if result.has("mode"):
...
func _on_message_received(client_id, message):
	_log("[DEBUG] message from xtoys client %d: %s" % [client_id, message])
	var result = JSON.parse_string(message)
	_log("[DEBUG] Parsed JSON result type: %s" % typeof(result))
	if typeof(result) != TYPE_DICTIONARY:
		_log("[DEBUG] Invalid xtoys JSON - expected Dictionary, got %s" % typeof(result))
		return
	# Queue the message for batch processing
	message_queue.append(result)

func _handle_position(cmd):
	_log("[DEBUG] Position command received: %s" % cmd)
	...
	var auxiliary = 0
	_log("[DEBUG] Sending smooth move: duration=%d, depth=%d, trans=%d, ease=%d" % [duration, depth, trans, ease])
	_send_smooth_move(duration, depth, trans, ease, auxiliary)

func _handle_speed(cmd):
	_log("[DEBUG] Speed command received: %s" % cmd)
	# speed is ms per stroke, upper/lower are 0-100
	var speed = int(cmd.get("speed", 1000))
	if speed == 0:
		_log("[DEBUG] Ignoring speed command with speed=0")
		xtoys_speed_zero = true
		_stop_speed_mode()
		return
	xtoys_speed_zero = false
	...
	_log("[DEBUG] Speed mode params: speed=%d, upper=%d, lower=%d" % [speed, speed_upper, speed_lower])
	if not speed_mode_active:
		_log("[DEBUG] Starting speed mode")
		_start_speed_mode()
	else:
		_log("[DEBUG] Speed mode already active")

func _start_speed_mode():
	if xtoys_speed_zero:
		_log("[DEBUG] xtoys_speed_zero is true, not starting speed mode")
		return
...
func _on_speed_timer_timeout():
	if xtoys_speed_zero:
		_log("[DEBUG] xtoys_speed_zero is true, blocking speed mode smooth move")
		return
...
func _send_smooth_move(ms_duration:int, depth:int, trans:int, ease:int, auxiliary:int):
	...
	_log("[DEBUG] Sending smooth move command: duration=%d, depth=%d, trans=%d, ease=%d, aux=%d" % [ms_duration, depth, trans, ease, auxiliary])
	...
	_log("[DEBUG] Command bytes: %s" % command.hex_encode())
	if %WebSocket.ossm_connected:
		_log("[DEBUG] WebSocket connected, broadcasting command")
		%WebSocket.server.broadcast_binary(command)
	else:
		_log("OSSM not connected, cannot send command") 

...
func _on_play_pause_button_pressed() -> void:
	if video_playback.is_open():
		if video_playback.is_playing:
			...
			if Global.main_scene.has_method("pause"):
				Global.main_scene.pause()
		else:
			video_playback.play()
...
func _try_load_funscript_for_video(video_path: String):
	...
	if FileAccess.file_exists(funscript_path):
		...
		if Global.main_scene.has_method("load_path_from_directory"):
			# Load funscript from the same directory as the video
			var success = Global.main_scene.load_path_from_directory(funscript_filename, dir + "/")
			if success:
